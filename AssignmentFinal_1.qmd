...
```{python}
# Install packages needed to run
...
```{python}
print("=" * 80)
print("Merge datasets on country, iso3c, year")
print("=" * 80)
...
```{python}
print("=" * 80)
print("Create df_dev for developing countries")
print("=" * 80)
...
def summarize_data(df_dev):
    # Descriptive statistics
    summary_stats = df_dev[['gdp_pc', 'realloc_gap', 'digital_intensity']].describe()
    
    # Correlation matrix
    correlation = df_dev[['gdp_pc', 'realloc_gap', 'digital_intensity']].corr()
...
print("=" * 80)
print("COMPREHENSIVE SUMMARY STATISTICS FOR DEVELOPING ECONOMIES (1990-2023)")
print("=" * 80)

# Basic Dataset Overview 
print("\n=== DATASET OVERVIEW ===\n")
print(f"Total observations: {len(df_dev):,}")
print(f"Number of countries: {df_dev['iso3c'].nunique()}")
print(f"Time period: {df_dev['year'].min()}–{df_dev['year'].max()}")
print(f"Total years covered: {df_dev['year'].nunique()}")
print(f"\nIncome group distribution:")
print(df_dev.groupby('income_group')['iso3c'].nunique().to_frame('n_countries'))

# Descriptive Statistics for Key Variables
print("\n=== DESCRIPTIVE STATISTICS (All Years) ===\n")
desc_vars = ["gdp_pc", "agr_share", "srv_share", "digital_intensity", 
             "realloc_gap", "internet_users", "mobile_subs"]
desc_stats = df_dev[desc_vars].describe().T
desc_stats['cv'] = (desc_stats['std'] / desc_stats['mean']).round(3)  # Coefficient of variation
desc_stats['range'] = desc_stats['max'] - desc_stats['min']
print(desc_stats.round(2))

# First vs Last Decade
print("\n=== STRUCTURAL CHANGE: 1990s vs 2020s ===\n")
early = df_dev[df_dev['year'].between(1990, 1999)]
late = df_dev[df_dev['year'].between(2020, 2023)]

comparison = pd.DataFrame({
    '1990s_Mean': early[['agr_share', 'srv_share', 'gdp_pc', 'digital_intensity']].mean(),
    '2020s_Mean': late[['agr_share', 'srv_share', 'gdp_pc', 'digital_intensity']].mean()
})
comparison['Absolute_Change'] = comparison['2020s_Mean'] - comparison['1990s_Mean']
comparison['Percent_Change'] = ((comparison['2020s_Mean'] / comparison['1990s_Mean'] - 1) * 100).round(1)
print(comparison.round(2))

# Regional Breakdown
print("\n=== REGIONAL STATISTICS (Latest Available Year) ===\n")
latest_year = df_dev['year'].max()
regional = (df_dev[df_dev['year'] == latest_year]
            .groupby('region')[['agr_share', 'srv_share', 'digital_intensity', 'gdp_pc']]
            .agg(['mean', 'median', 'std'])
            .round(2))
print(regional)

# Income Group Analysis
print("\n=== INCOME GROUP STATISTICS (Latest Available Year) ===\n")
income_stats = (df_dev[df_dev['year'] == latest_year]
                .groupby('income_group')[['agr_share', 'srv_share', 'digital_intensity', 'realloc_gap']]
...
print("\n=== I. KEY CORRELATIONS ===\n")
corr_vars = ['digital_intensity', 'agr_share', 'srv_share', 'realloc_gap', 
             'log_gdp_pc', 'service_ratio']
correlations = df_dev[corr_vars].corr()['digital_intensity'].sort_values(ascending=False)
print(correlations.round(3))

# Growth Rate Statistics 
print("\n=== AVERAGE ANNUAL GROWTH RATES (% per year) ===\n")
growth_stats = (df_dev.groupby('country')
                .apply(lambda g: pd.Series({
                    'digital_growth_mean': g['digital_growth'].mean(),
                    'srv_share_cagr': ((g['srv_share'].iloc[-1] / g['srv_share'].iloc[0]) ** 
                                       (1 / (g['year'].iloc[-1] - g['year'].iloc[0])) - 1) * 100
                                       if len(g) > 1 and g['srv_share'].iloc[0] > 0 else np.nan
                }))
                .describe()
                .round(2))
print(growth_stats)
...
df_dev['decade'] = (df_dev['year'] // 10) * 10
completeness = (df_dev.groupby('decade')[key_vars]
                .apply(lambda x: (x.notna().sum() / len(x) * 100).round(1))
                .T)
print(completeness)
```
...
```{python}
# Plot: Mobile Subscriptions vs Internet Users (2023)
import matplotlib.pyplot as plt
import numpy as np

# Prepare data for 2023
df_2023 = df_dev[df_dev["year"] == 2023].dropna(subset=["mobile_subs", "internet_users"])

# Create scatter plot
fig, ax = plt.subplots(figsize=(10, 6))

# Color by income group
colors = {"Low Income": "#E74C3C", 
          "Lower Middle Income": "#F39C12", 
          "Upper Middle Income": "#27AE60"}

for group in df_2023["income_group"].unique():
    subset = df_2023[df_2023["income_group"] == group]
    ax.scatter(subset["mobile_subs"], subset["internet_users"], 
               label=group, alpha=0.7, s=60, color=colors.get(group, "gray"))

# Add diagonal reference line (where mobile = internet)
max_val = max(df_2023["mobile_subs"].max(), df_2023["internet_users"].max())
ax.plot([0, max_val], [0, max_val], 'k--', alpha=0.3, linewidth=1, label="Equal adoption")

# Highlight South Africa
sa = df_2023[df_2023["iso3c"] == "ZAF"]
if not sa.empty:
    ax.scatter(sa["mobile_subs"], sa["internet_users"],
               s=200, marker="*", edgecolor="k", linewidth=1.5, 
               color="yellow", label="South Africa", zorder=5)
    ax.text(float(sa["mobile_subs"]) + 2, float(sa["internet_users"]) + 2,
            "South Africa", fontsize=9, fontweight='bold')

# Formatting
ax.set_xlabel("Mobile Subscriptions (per 100 people)", fontsize=12)
ax.set_ylabel("Internet Users (% of Population)", fontsize=12)
ax.set_title("Mobile vs Internet Adoption in Developing Economies (2023)\nColoured by Income Group", 
...
# Print insights
print("\n" + "=" * 60)
print("MOBILE vs INTERNET ADOPTION INSIGHTS (2023)")
print("=" * 60)

for group in ["Low Income", "Lower Middle Income", "Upper Middle Income"]:
    subset = df_2023[df_2023["income_group"] == group]
    if len(subset) > 0:
        print(f"\n{group}:")
        print(f"  Avg Mobile Subs:    {subset['mobile_subs'].mean():.1f} per 100")
        print(f"  Avg Internet Users: {subset['internet_users'].mean():.1f}%")
        print(f"  Gap:                {subset['mobile_subs'].mean() - subset['internet_users'].mean():.1f}")
        print(f"  N:                  {len(subset)} countries")
```

```{python}

# Plot: Internet Adoption by Income Group (2000 vs 2023)
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
# Calculate mean internet users by income group and year
adoption = (df_dev[df_dev["year"].isin(years)]
            .dropna(subset=["income_group", metric])
            .groupby(["income_group", "year"])[metric]
            .mean()
            .unstack(fill_value=0))

# Order income groups from low to high
order = ["Low Income", "Lower Middle Income", "Upper Middle Income"]
adoption = adoption.reindex(order).dropna(how="all")

# Create bar chart
fig, ax = plt.subplots(figsize=(10, 6))

x = np.arange(len(adoption.index))
width = 0.35


bars1 = ax.bar(x - width/2, adoption[2000], width, label="2000", color="#3498DB", alpha=0.8)
bars2 = ax.bar(x + width/2, adoption[2023], width, label="2023", color="#E74C3C", alpha=0.8)
...
# Formatting
ax.set_xticks(x)
ax.set_xticklabels(adoption.index, fontsize=11)
ax.set_ylabel("Internet Users (% of Population)", fontsize=12)
ax.set_title("Digital Divide: Internet Adoption by Income Group (2000 vs 2023)", 
             fontsize=13, pad=15, fontweight='bold')
ax.legend(fontsize=11, frameon=False)
ax.grid(axis="y", alpha=0.3, linestyle='--')
ax.set_ylim(0, max(adoption.max()) * 1.15)  
plt.tight_layout()
plt.show()
...
print("=" * 80)
print("Cross-sectional OLS (2023)")
print("=" * 80)

# Prepare data for 2023
df_reg = df_dev[df_dev['year'] == 2023][['country', 'digital_intensity', 'realloc_gap']].dropna()

print(f"\nSample: {len(df_reg)} countries in 2023\n")

# Run simple linear regression
X = df_reg['digital_intensity'].values
y = df_reg['realloc_gap'].values

slope, intercept, r_value, p_value, std_err = linregress(X, y)

# Print results
print("Regression Equation:")
print(f"  Reallocation Gap = {intercept:.2f} + {slope:.2f} × Digital Intensity\n")
...
print(f"\nInterpretation:")
print(f"  A 0.1 increase in digital intensity is associated with a")
print(f"  {slope*0.1:.2f} percentage point change in the reallocation gap.")

# Visualization
fig, ax = plt.subplots(figsize=(10, 6))

ax.scatter(X, y, alpha=0.6, s=60, edgecolors='k', linewidth=0.5)
...
ax.set_xlabel('Digital Intensity (0-1)', fontsize=12)
ax.set_ylabel('Reallocation Gap: Services - Agriculture (% GDP)', fontsize=12)
ax.set_title('Digital Connectivity and Structural Transformation (2023)', 
